...
if uploaded:
    ...
    col4.metric("Avg AOV", f"₹{filtered['AOV'].mean():.0f}")
    col5.metric("Avg Ad Contribution", f"{filtered['Ads_CV_pct'].mean():.1f}%")
    ...
    for _, row in filtered.iterrows():
        issues = []
        if row['Orders'] == 0 and row['Month'] == latest_month:
            issues.append("Zero orders this month")
        if row['SL_pct'] < 85:
